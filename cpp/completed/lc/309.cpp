...
public:
  ...
  auto T(int i, int p, int bought, bool sold_yesterday) -> int{
    if(i>=prices.size()) {
      return p;
    }
    // do nothing
    int res = T(i+1, p, bought, false);
    //selling
    if(bought > 0){
      res = max(res, T(i+1, p+prices[i], bought-1, true));
    }
    // buying
    if( !sold_yesterday ){
      res = max(res, T(i+1, p-prices[i], bought+1, false));
    }
        return res;
  }
  ...
  // bottom up dp
  auto T2() -> int{
   for(int i=dp.size()-2; 0<=i; --i){           //for each day starting from the end
    for(int j = i+1; j<dp.size(); ++j){         // each prossible sell
      int jump_pos = min(static_cast<size_t>(j+2), dp.size()-1); 
      dp[i] = max(dp[i], prices[j]-prices[i] + dp[jump_pos]);
    }
    dp[i] = max(dp[i], dp[i+1]);                // the case of not buying 
   }
   return dp[0];
  }
  ...
  int maxProfit(vector<int>& prices) {
    this->prices = prices;
    this->dp = vector<int>(prices.size(), 0);
    return T2();
  }
};
